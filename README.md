# C Programming

A structured collection of C programming problems created while
learning and strengthening C programming fundamentals.

## Topics Covered

1. C Basics
2. Loops & Patterns
3. Functions & Recursion
4. Arrays
5. Strings
6. Pointers
7. Structures & Unions
8. Dynamic Memory Allocation
9. File Handling
10. Interview-Level Problems

## Learning Goals

- Build strong C programming fundamentals
- Improve problem-solving skills
- Understand memory and pointers
- Practice data structures and algorithms
- Develop programming fundamentals useful for VLSI and digital design

## Progress

| Level | Topic | Problems | Status |
|---|---|---:|---|
| 1 | C Basics | 1–10 | 🔄 In Progress |
| 2 | Loops & Patterns | 11–15 | 🔒 Not Started |
| 3 | Functions | 16–20 | 🔒 Not Started |
| 4 | Arrays | 21–28 | 🔒 Not Started |
| 5 | Strings | 29–35 | 🔒 Not Started |
| 6 | Pointers | 36–40 | 🔒 Not Started |
| 7 | Structures & Unions | 41–43 | 🔒 Not Started |
| 8 | Dynamic Memory | 44–46 | 🔒 Not Started |
| 9 | File Handling | 47–48 | 🔒 Not Started |
| 10 | Interview-Level C | 49–50 | 🔒 Not Started |

## Note

All programs are written for learning and practice purposes.
